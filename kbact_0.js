...
var k1 = {8:'BackSpace',13:'Enter',16:'shift',17:'ctrl',18:'alt',32:'Space',37:'←',38:'↑',39:'→',40:'↓',189:'-'};
var cl = false;var score = 0;var keyname = [];var alpb = 'abcdefghijklmnopqrstuvwxyz';
for (i=0;i<36;i++){if (i<10) k1[(i + 48)] = i.toString();else k1[(i + 55)] = alpb[i - 10];}
function sum(list){var n = 0;for (i=0;i<list.length;i++) n += list[i];return n;}
var key = {};var k0 = Object.keys(k1);k0.sort();for(var i=0;i<k0.length;i++) key[ k0[i] ] = k1[ k0[i] ];
if (lst.getItem('#keyrank') === null){
    ...
    var olds = ['-', '2x', '4x', 't/2', '+5'];var json = [];var js0 = JSON.parse(lst.getItem('#keyrank'));
    var bl_1 = false;for (i=0;i<Object.keys(js0).length;i++) json.push(js0[i + 1]["option"].length);
    if (sum(json) > json.length){
        for (i=0;i<Object.keys(json).length;i++){
            var itms = js0[i + 1]["option"];var litms = itms.split(', ');var l_0 = [];
            for (j=0;j<litms.length;j++){
                if (litms[j] === '1/2') litms[j] = 't/2';
                else if (litms[j] === '3x') litms[j] = '4x';
...
function course(x){
    ...
    if (!cl){opt = opt.filter(function (x, i, self) {return self.indexOf(x) === i;});if (b_s) keyselect();else startcheck(x);}
    else{cl = false;}types = parseInt(x);
}
function reset(){
...
function keyplus(){if (keyin){if (b1 && keynums[0] !== "mouse"){if (keynums.indexOf(event.keyCode) >= 0) times ++;document.getElementById('showtimes').innerHTML = times;}}}
function keyplus_1(){if (keyin){if (keynums[0] === 'mouse'){times ++;document.getElementById('showtimes').innerHTML = times;se2 = setInterval(function (){document.getElementById('hidbtn').focus();}, 1);}}}
...
function pressKeys(){
    b1 = true;keyin = true;var left = 10;
    if (p5) left += 5;
    if (half) left = Math.floor(left / 2);
    if (trp) left = Math.floor(left * 0.4);
...
function result_format(){
    ...
    if (first_0){
        ...
        if (total > parseInt(json[1]["score"])){
            ...
            json[1]["push"] = s1;
            json[1]["course"] = types;
            json[1]["option"] = parseInt(opt_now.map(function (x){return x.toString();}).join(''));
            ...
            json[2]["push"] = s1;
            json[2]["course"] = types;
            json[2]["option"] = parseInt(opt_now.map(function (x){return x.toString();}).join(''));
            ...
            json[3]["push"] = s1;
            json[3]["course"] = types;
            json[3]["option"] = parseInt(opt_now.map(function (x){return x.toString();}).join(''));
...
function showresults(){first_0 = true;
    document.getElementById('main').innerHTML = '<p class="msg_c">スコアを計算中！<br>ちょっと待っててね。</p>';s1 = times;
